...
import { shouldRequestDeliveryResumeWake } from './scheduler-wake';
 ...
 * @property requestSchedulerWake Event append または Delivery resume 後に Run scheduler を起こす callback です。
 ...
 * Delivery result 後の scheduler wake 判定もここへ寄せ、AIAgent class には public method 名だけを残します。
 *
 * @example
 * ```ts
...
function publishDeliveryResult(
  ...
  const result = publishIntegrationDeliveryResultInStore({
    agentId: context.agentId,
    command,
    repositories: context.repositories,
  });
  void result
    .then((deliveryResult) => {
      // follow-up Event または resume が必要な新規 Delivery callback だけ scheduler wake を要求します。
      if (shouldRequestDeliveryResumeWake(deliveryResult)) {
        context.requestSchedulerWake({
          reason: 'event_accepted',
          requestedAtMs: command.context.requestedAtMs,
        });
