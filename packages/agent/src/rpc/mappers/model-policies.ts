 ...
 * @example
 * ```ts
 * const input = mapModelPolicyCommandInput(request.policy);
 * ```
 */
...
/**
 * UpsertModelPolicy の domain result を generated response 初期化値へ変換します。
 *
 * @param result upsert 後 policy、validation、任意 audit、replay 状態を含む domain result です。
 * @returns `UpsertModelPolicyResponseSchema` に渡せる plain object です。
 * @throws この関数は domain result の純粋変換だけを行うため例外を投げません。
 * @example
 * ```ts
 * const response = mapUpsertModelPolicyResponse(result);
 * ```
 */
...
/**
 * GetModelPolicy の domain view を generated response 初期化値へ変換します。
 *
 * @param policy Agent-owned storage から読んだ model policy view です。
 * @returns `GetModelPolicyResponseSchema` に渡せる plain object です。
 * @throws この関数は policy view の写像だけを行うため例外を投げません。
 * @example
 * ```ts
 * const response = mapGetModelPolicyResponse(policy);
 * ```
 */
...
/**
 * ListModelPolicies の domain result を generated response 初期化値へ変換します。
 *
 * @param result Agent-scoped page metadata と model policy view 配列を含む domain result です。
 * @returns `ListModelPoliciesResponseSchema` に渡せる plain object です。
 * @throws この関数は list result の純粋変換だけを行うため例外を投げません。
 * @example
 * ```ts
 * const response = mapListModelPoliciesResponse(result);
 * ```
 */
...
/**
 * ArchiveModelPolicy の domain result を generated response 初期化値へ変換します。
 *
 * @param result archive 後 policy、任意 audit、replay 状態を含む domain result です。
 * @returns `ArchiveModelPolicyResponseSchema` に渡せる plain object です。
 * @throws この関数は mutation 結果の純粋変換だけを行うため例外を投げません。
 * @example
 * ```ts
 * const response = mapArchiveModelPolicyResponse(result);
 * ```
 */
...
/**
 * ValidateModelPolicy の domain result を generated response 初期化値へ変換します。
 *
 * @param result validation result と任意の policy preview を含む domain result です。
 * @returns `ValidateModelPolicyResponseSchema` に渡せる plain object です。
 * @throws この関数は validation view の純粋変換だけを行うため例外を投げません。
 * @example
 * ```ts
 * const response = mapValidateModelPolicyResponse(result);
 * ```
 */
...
/**
 * Model policy summary view を generated message 初期化値へ変換します。
 *
 * @param policy secret-free な model policy summary view です。
 * @returns generated ModelPolicySummary 相当の plain object です。
 * @throws この関数は summary view の写像だけを行うため例外を投げません。
 * @example
 * ```ts
 * const summary = mapAgentModelPolicySummary(policy);
 * ```
 */
...
/**
 * Model policy validation view を generated message 初期化値へ変換します。
 *
 * @param validation 任意の validation view です。
 * @returns generated validation message 相当の plain object、または入力未指定時は `undefined` です。
 * @throws この関数は validation view の写像だけを行うため例外を投げません。
 * @example
 * ```ts
 * const validation = mapAgentModelPolicyValidation(result.validation);
 * ```
 */
...
/**
 * Full model policy view を generated message 初期化値へ変換します。
 *
 * @param policy Agent-owned storage から組み立てた full model policy view です。
 * @returns generated ModelPolicy message 相当の plain object です。
 * @throws この関数は policy view の写像だけを行うため例外を投げません。
 * @example
 * ```ts
 * const message = mapAgentModelPolicy(policy);
 * ```
 */
