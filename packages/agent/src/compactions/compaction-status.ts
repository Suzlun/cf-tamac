 ...
 *
 * @remarks
 * 状態遷移を検証する関数へ渡す小さな value object です。未知 status 文字列は事前に
 * `assertCompactionStatus` で検証してからこの型にします。
 *
 ...
 * @example
 * ```ts
 * const shouldContinue = isUnfinishedCompactionStatus(row.status);
 * ```
 */
 ...
 * @returns TypeScript の型を `CompactionStatus` へ narrow します。
 * @throws TypeError 未知の status が渡された場合に発生します。
 * @example
 * ```ts
 * assertCompactionStatus(row.status);
 * ```
 */
 ...
 * @example
 * ```ts
 * assertCompactionStatusTransition({ from: 'pending', to: 'running' });
 * ```
 */
