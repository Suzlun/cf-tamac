...
import { ensureAgentIntegrationTables } from './integration';
import { ensureAgentToolTables } from './tool';

import type { AgentStorageDatabase } from '../database';

/**
 * `AIAgent` instance が所有する全 Agent foundation SQLite tables を作成します。
 *
 * @remarks
 * Durable Object SQLite schema creation は Agent storage で許可する狭い handwritten SQL 例外、
 * すなわち `single narrow handwritten SQL exception` です。
 * Drizzle Durable Object adapter は typed query execution を提供しますが、runtime migration は行わないため、
 * table creation だけを initializer に閉じ、通常の読み書きは repository の query builder に閉じます。
 * @param database AIAgent Durable Object の SQLite に接続した Drizzle adapter です。
