...
`thread_key` は Integration または Client が指定する opaque string である。異なる Integration が意図的に同じ文字列を指定した場合、その Event は同じ Thread に統合される。
...
thread_key 付き AgentEvent として Agent が受理する
  ↓
Agent が Thread の文脈、State、Memory、Schedule、Tool、Integration を参照する
  ↓
AgentRun を開始する
  ↓
...
- Integration や Tool の実装を Agent package に固定的に埋め込むこと
...
AgentIntegrationService.InstallIntegration(agent_id, ...)
```
...
- Integration の install/uninstall を Agent RPC 経由で操作する
...
Agent lifecycle や Integration install の監査 Event は、AIAgent 内部の予約済み system Thread に所属させる。
...
異なる Integration、Adapter、Connection が同じ文字列を指定しても、同じ Thread に統合する。これは明示的な文脈統合である。
...
| Integration | Adapter と Tool の定義を追加できる配布単位 |
| Installation | ある Agent に Integration を導入した状態 |
| Connect | Protobuf RPC を HTTP 上で運ぶ transport profile |
| native gRPC | HTTP/2 と trailer を使う任意互換 transport profile |
| Principal | Client Service、Integration Installation、Internal Service などの認証主体 |
...
flowchart LR
    ...
    Provider[Integration Provider Server]
...
| Integration Provider | Connect | binary Protobuf | installation signature 必須 |
...
- Cloudflare Worker、Next.js、Integration Provider 間の互換性が高い
...
bindings:
  ...
  integration trust / signing material
```
...
### 8.3 Integration Provider

Integration Provider は Agent Service と別ホストにできる。

```text
https://discord-integration.example.com
https://slack-integration.example.com
https://github-integration.example.com
```
...
### 9.10 IntegrationIngressService

Integration Installation principal が呼び出せる service を限定する。
...
Integration は Agent config、install/uninstall、approval RPC を呼べない。
...
- Integration Installation の最終認可
- Memory 更新
- Compaction
- AgentRun scheduling
...
### 11.3 Integration Installation authentication

Integration Provider から Agent への RPC は Installation 単位の detached signature を標準とする。
...
- Integration Provider の環境によって client certificate 運用可否が異なる

外部 Provider 専用 hostname で mTLS を追加する場合も、Installation signature と Agent grant の検証は省略しない。

### 11.7 RPC authorization matrix

| RPC group | CLIENT_SERVICE | INTEGRATION_INSTALLATION | INTERNAL_SERVICE |
|---|---:|---:|---:|
...
| Integration install/uninstall | allow by scope | deny | deny |
...
export class AIAgent extends Agent<Env, AgentState> {
  ...
  async installIntegration(input: InstallIntegrationCommand): Promise<InstallationView>;
...
Agent lifecycle、Integration install/uninstall、credential rotation など Thread を持たない管理操作は予約済み system Thread に Event として記録する。
...
- Integration Installation version
...
### 19.4 Integration uninstall

Schedule metadata に `installation_id` を保持する。

Integration uninstall 時、その Installation が作成した active Schedule を cancel する。
...
Integration Provider が Tool を提供する場合、同じ Protobuf RPC-only 原則を適用する。
...
- IntegrationIngressService.PublishEvent 呼び出し
...
sequenceDiagram
    ...
    UI->>C: Install integration
    C->>R: InstallIntegration protobuf RPC
    R->>A: installIntegration(command)
    A->>P: fetch/verify manifest
    A->>A: persist installation + grants + trust
    A-->>R: installation / setup instructions
    R-->>C: InstallIntegrationResponse
...
4. Integration Tool を disable
5. pending ToolInvocation を cancel または outcome_unknown
6. Integration 由来 Schedule を cancel
...
## 23. Integration protocol と署名

### 23.1 二段階の信頼境界

```text
External platform → Integration Provider
  platform-specific verification

Integration Provider → Agent Service
...
- Agent IntegrationIngressService への signed Connect RPC
...
- Integration Installation snapshot
...
5. Integration/tool/schedule UI

### Stage 9: Discord Integration
...
- Agent-owned Schedule/Tool/Integration
- Client-owned D1
...
- [ ] Schedule/ToolInvocation/Integration Installation が Agent DO にある
