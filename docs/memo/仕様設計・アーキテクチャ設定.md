...
`packages/client` は Next.js 製の簡易管理クライアントである。Client 専用 D1 を読み書きし、管理対象 Agent ID、Agent RPC origin、表示設定、接続資格情報の参照を管理する。Agent は Client D1 にアクセスしない。
...
```text
/aia.agent.v1.AgentEventService/PublishEvent
/aia.agent.v1.AgentThreadService/GetThread
/aia.agent.v1.AgentExtensionService/InstallExtension
```
...
packages/agent/proto/aia/agent/v1/*.proto
...
{
  "scripts": {
    "gen:agent:proto": "pnpm --filter @aia/agent gen:proto",
    "gen:agent:rpc": "pnpm --filter @aia/agent gen:rpc",
