# tamac-sdk Specification

## Purpose

TAMAC の server-side SDK、Provider ingress security、Client Service 認証、Browser-safe result、origin policy を、Agent TypeSpec/RPC contract に従う typed execution boundary として定義する。
...
Provider ingress は、信頼済み edge source identity と RPC procedure から構成される request identity に対して、Cloudflare RateLimit binding による pre-auth traffic allowance を評価する SHALL。RateLimit binding の外部 bucket は edge admission state に限定し、Agent domain state の正本や Client D1 の状態として扱って MUST NOT。

Provider ingress traffic が allowance を超えた場合、Agent Service は detached-signature の有効性に依存しない安全な `resource_exhausted` response を返す SHALL。後続の認可済みreadはrequest受信前と同じAgent resource versionを返す SHALL。

Provider ingress の accepted Event、pending Run、Thread identity、replay/idempotency、audit、Agent resource version は AIAgent Durable Object SQLite が authoritative に所有する SHALL。pre-auth denial は raw body、signature、Agent domain mutation より前に完了する SHALL。
...
- **AND** Provider ingress traffic が信頼済み edge source identity と RPC procedure に対する allowance を超えた場合、Agent Service は detached-signature の有効性に依存しない安全な `resource_exhausted` response を返す
...
Mutation response から適用状態を確定できない場合、SDK consumer は同じ idempotency context による状態確認を次操作として提示し、利用者入力と直前に確認済みの表示状態を保持する safe failure result を返す SHALL。

Managed Agent registration の状態確認は、`GetAgent` が返す initialization receipt の idempotency key と registration request digest、profile、config が登録試行と完全一致した場合だけ `active` を確定 SHALL。`not_found` は同じ attempt の Client-owned metadata と credential reference を atomic cleanup し、destroyed、missing receipt、部分一致、query error は `reconciliation_required` を保持 SHALL。
...
- **AND** Managed Agent registration は `GetAgent` の initialization receipt と登録試行の idempotency key、registration request digest、profile、config が完全一致した結果で `active` として確定する
