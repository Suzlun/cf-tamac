...
- [ ] 1.1 Read `proposal.md`, `design.md`, `specs/management-client/spec.md`, `specs/client-management/spec.md`, and `wireframes/*.md`; pin `/` → `/agents`, `/settings`, no `/agents/new`, no top-level Tools/Compactions, and no Topbar quick switcher as implementation decisions.
...
- [ ] 2.1 Update `packages/client/app/page.tsx` so `/` redirects to `/agents`; verify no landing/demo content remains.
...
- [ ] 2.3 Add `packages/client/src/components/sidebar-navigation.tsx` with Global area (`Agents`, `Global Settings`) and selected-Agent area (`Overview`, `Threads`, `Events`, `Runs`, `Schedules`, `Integrations`, `Settings`) plus hidden/disabled Agent-unselected state.
...
- [ ] 3.4 Delete `packages/client/app/agents/new/page.tsx` and remove every sidebar or route-manifest expectation that treats `New Agent` as an independent screen.
...
- [ ] 4.2 Update `packages/client/app/agents/[agentId]/threads/page.tsx` so Thread detail includes Memory/Compaction metadata and does not require a separate Compactions screen.
...
- [ ] 4.10 Update `packages/client/src/components/tool-view.tsx` and `packages/client/src/components/compaction-view.tsx` so they are detail components without standalone route assumptions.
...
- [ ] 6.1 Add/update `packages/client/src/tests/management-navigation.test.tsx` with a test titled `[MANAGEMENT-CLIENT-S001] Agent registry shell renders for a browser user` covering `/agents`, registration action, selected-Agent disabled state, and no demo content.
...
- [ ] 6.4 Add/update `packages/client/src/tests/management-navigation.test.tsx` with a test titled `[MANAGEMENT-CLIENT-S010] Selected-Agent area activates for registered Agent context` covering the seven selected-Agent items and no Topbar quick switcher.
- [ ] 6.5 Add/update `packages/client/src/tests/client-api-proxy-absence.test.ts` with a test titled `[MANAGEMENT-CLIENT-S011] Route manifest exposes only Management Client screens` covering `/settings`, removal of `/agents/new`, removal of `tools`/`compactions` routes, and no `/api/**` Agent proxy.
- [ ] 6.6 Add/update `packages/client/src/tests/management-navigation.test.tsx` with a test titled `[MANAGEMENT-CLIENT-S007] Management routes contain no demo navigation` covering no `hello`/`users`, no `New Agent` top-level item, and no `Tools`/`Compactions` top-level items.
...
- [ ] 6.9 Add/update `packages/client/src/tests/agent-registry-shell.test.tsx` with a test titled `[CLIENT-MANAGEMENT-S010] Agents screen owns Agent registration and selection` covering registration action, selection action, status labels, and no independent `/agents/new` route dependency.
...
- [ ] 6.11 Add/update `packages/client/src/tests/management-navigation.test.tsx` or dedicated component tests with a test titled `[CLIENT-MANAGEMENT-S012] Tools and Compactions are shown inside Agent-scoped context` covering Tool/Compaction metadata in Overview/Threads/Runs/Integrations/Settings and no standalone navigation.
