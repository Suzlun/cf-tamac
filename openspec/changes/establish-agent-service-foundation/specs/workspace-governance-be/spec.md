...
#### Scenario: Root generation commands produce deterministic Agent outputs (WORKSPACE-GOVERNANCE-BE-S001)
...
#### Scenario: Codegen check fails on Agent generated drift (WORKSPACE-GOVERNANCE-BE-S002)
...
#### Scenario: Protobuf field stability guard rejects unstable descriptors (WORKSPACE-GOVERNANCE-BE-S009)
...
#### Scenario: Lint rejects forbidden Agent API surface fixtures (WORKSPACE-GOVERNANCE-BE-S003)
...
#### Scenario: Lint rejects Agent and Client runtime coupling (WORKSPACE-GOVERNANCE-BE-S004)

- **GIVEN** fixture または test case が Agent runtime source から Client runtime source、または Client runtime source から Agent runtime source を import する
- **WHEN** workspace lint guardrails が実行される
- **THEN** guardrail は package-boundary violation を報告し、failure で終了する

#### Scenario: Scenario ID coverage validates foundation specs (WORKSPACE-GOVERNANCE-BE-S005)
...
実装担当者と reviewer は、`packages/agent/**` と `packages/client/**` がそれぞれ Agent Service と management Client の新しい作業単位であることを、subagent permission、delegation map、coding-guardian の entrypoint から判断できる必要がある。古い `packages/backend/**` / `packages/frontend/**` 前提だけが残ると、applier が誤った担当 agent へ委譲したり、engineer/reviewer が新しい path を規約外として扱ったりする。
...
`.opencode/skills/coding-guardian/references/repo-entrypoints.md` は、現在の backend/frontend/template entrypoints を置き換える Agent/Client entrypoints、codegen scripts、Worker configs、governance scripts を列挙 SHALL。
...
#### Scenario: OpenCode workflow recognizes Agent and Client foundations (WORKSPACE-GOVERNANCE-BE-S008)
...
- **THEN** `packages/agent/**` は backend-oriented implementation と review の Agent Service implementation scope として認識される
- **AND** `packages/client/**` は frontend-oriented implementation と review の management Client implementation scope として認識される
- **AND** generated RPC output paths は command-owned のままであり、手編集は許可されない
- **AND** `.opencode` workflow updates は古い backend/frontend template paths に暗黙依存せず governance verification で coverage される
...
#### Scenario: Documentation exposes Agent and Client foundation commands (WORKSPACE-GOVERNANCE-BE-S006)
...
#### Scenario: Supply-chain lint enforces release-age and build-script policy (WORKSPACE-GOVERNANCE-BE-S007)
