...
Agent platform は `hello` と `users` demonstration domains、および旧 demo package graph を active Agent contract と runtime graph の外に保つ MUST。
...
Replacement verification 後の active workspace は Agent contract source と runtime を `packages/agent/**` に集約 SHALL し、旧 demo contract/runtime packages を Agent implementation source、workspace package patterns、documented development commands、lint boundary elements として残して MUST NOT。
...
- **AND** Agent entrypoint は旧 demo contract/runtime packages を import/export しない
