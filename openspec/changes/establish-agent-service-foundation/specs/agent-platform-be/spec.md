...
Agent API contract は `packages/agent/src/typespec` を正本 SHALL とし、`aia.agent.v1` package の proto3 files を `packages/agent/proto/aia/agent/v1` へ emit SHALL。
...
#### Scenario: TypeSpec emits proto3 as the Agent contract (AGENT-PLATFORM-BE-S001)

- **GIVEN** Agent TypeSpec project が contract generation 用に compile される状態である
- **WHEN** generation command が完了する
- **THEN** `packages/agent/proto/aia/agent/v1` に common types、すべての Agent foundation models、lifecycle/event/thread/run/state/schedule/tool/extension/agent-adapter/health service files 由来の proto3 service/message files が存在する
...
#### Scenario: Protobuf field numbers and service methods are stable (AGENT-PLATFORM-BE-S014)
...
#### Scenario: Binary Connect requests are accepted and JSON requests are rejected (AGENT-PLATFORM-BE-S002)
...
#### Scenario: REST and Orval Agent surfaces are unreachable (AGENT-PLATFORM-BE-S003)
...
#### Scenario: Public RPC descriptors are agent-scoped and exclude cross-Agent list/search (AGENT-PLATFORM-BE-S010)

- **GIVEN** `aia.agent.v1` の generated proto と Protobuf-ES service descriptors が利用できる
...
#### Scenario: Command and Event publish descriptors require replay and Thread keys (AGENT-PLATFORM-BE-S011)
...
#### Scenario: Thread key identity is normalized and Agent-scoped (AGENT-PLATFORM-BE-S013)
...
#### Scenario: Agent ID resolves to a single AIAgent instance (AGENT-PLATFORM-BE-S004)
...
#### Scenario: Agent Worker bindings exclude Client D1 and Cloudflare Queues (AGENT-PLATFORM-BE-S005)
...
#### Scenario: Agent-local Queue coalesces scheduler wakes without owning events (AGENT-PLATFORM-BE-S012)
...
Agent platform は `hello` と `users` demonstration domains を active Agent contract と runtime graph の外に保つ MUST。
...
Agent platform runtime、contract sources、package exports、tests は `hello` と `users` domain modules、routes、schemas、TypeSpec models、TypeSpec routes、Drizzle tables、generated Agent client wrappers を含まない SHALL。
...
#### Scenario: Demo resource paths are not served by the Agent Worker (AGENT-PLATFORM-BE-S006)
...
#### Scenario: Demo domain files are not reachable from Agent entrypoints (AGENT-PLATFORM-BE-S007)
...
#### Scenario: Health RPC reaches the Connect Worker facade (AGENT-PLATFORM-BE-S008)
...
#### Scenario: Foundation handlers fail closed for unmapped methods (AGENT-PLATFORM-BE-S009)
