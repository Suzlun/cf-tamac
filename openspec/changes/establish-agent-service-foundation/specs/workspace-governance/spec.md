...
Workspace lint は、旧 demo server/client package に課していた依存方向の規律を Agent/Client の active package graph へ移植 SHALL。Agent 側は Worker entrypoint、RPC facade、service modules、Agent domain/runtime modules、Agent-owned storage/generated RPC の方向を検査 SHALL。Client 側は Next.js App Router、browser-visible modules、Server Components/Server Actions、server-only modules、Client D1 repositories/generated Agent RPC client の方向を検査 SHALL。

Workspace lint と documentation checks は、旧 demo package graph を replacement verification 後の active package boundary として扱って MUST NOT。
...
実装担当者と reviewer は、`packages/agent/**` と `packages/client/**` がそれぞれ Agent Service と management Client の新しい作業単位であることを、subagent permission、delegation map、coding-guardian の entrypoint から判断できる必要がある。旧 demo package 前提だけが残ると、applier が誤った担当 agent へ委譲したり、engineer/reviewer が新しい path を規約外として扱ったりする。
...
`.opencode/skills/coding-guardian/references/repo-entrypoints.md` は、旧 demo template entrypoints を置き換える Agent/Client entrypoints、codegen scripts、Worker configs、governance scripts を列挙 SHALL。
...
- **AND** `.opencode` workflow updates は旧 demo template paths に暗黙依存せず governance verification で coverage される
...
Repository documentation は、replacement verification 後の active development units を `packages/agent/**` と `packages/client/**` として説明 SHALL し、旧 demo package categories を primary architecture、dependency direction、development commands として説明 MUST NOT。
