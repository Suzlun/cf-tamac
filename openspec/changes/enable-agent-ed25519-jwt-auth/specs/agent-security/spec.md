...
Agent API は Client Service、Integration Installation、Internal Service、Admin Operator など複数の主体から呼ばれる。ブラウザーユーザーを直接 Agent principal にせず、サーバー側 service identity と acting user 情報で監査できる必要がある。Client Service 認証は自己ホスト運用で初期設定用 RPC や installer secret に依存せず、Agent Worker の信頼設定と Management Client の署名鍵管理で成立する必要がある。
...
- 認証メタデータは decode 済みリクエストに bind MUST し、生 body digest とともに最終認可のため AIAgent Durable Object へ forward MUST。

#### Scenario: 有効な Client Service JWT が Agent RPC を認証する (AGENT-SECURITY-S001)

- **GIVEN** Client Service が `agent-alpha` 用の active Ed25519 signing key を保持し、Agent Worker の `AGENT_CONTROL_PLANE_TRUST` が対応する issuer/kid の公開鍵と policy を含んでいる
...
Client と Provider は、失敗時に再試行すべきか、入力を直すべきか、認可を確認すべきかを Connect code で判断する。運用者は agent/thread/run/tool/installation 単位で問題を追跡できる必要があるが、秘密値は log に出してはならない。認証失敗と認可失敗は原因分類だけを安全に観測でき、token body や key material を含まない必要がある。
...
#### Scenario: ドメイン error が安定した Connect code へ対応付けられる (AGENT-SECURITY-S007)
...
- **THEN** 各 error は構成済み Connect code と安全なエラー詳細に対応付けられる
- **AND** Client は再試行可能な分類と再試行不可の分類を区別できる

#### Scenario: 観測文脈が secret material を除外する (AGENT-SECURITY-S008)
...
自己ホスト運用者は Agent Worker の Variables and Secrets に公開情報だけの信頼設定を設定し、複数 issuer と複数署名鍵を明示的に管理したい。設定が壊れている、key が失効している、または想定外の issuer/kid が使われている場合、Agent は安全側に倒れて状態変更を受け付けない必要がある。
...
- Agent Service は trust config の parse error、schema validation error、unknown issuer、unknown kid、unsupported key type、missing policy を拒否 MUST。
...
#### Scenario: 信頼設定の不正状態は安全側で拒否される (AGENT-SECURITY-S011)

- **GIVEN** trust config が JSON として不正、schema 不一致、private key parameter `d` を含む、未知の issuer/kid を要求される、または key status が `revoked` である
- **WHEN** Client Service が Agent RPC を呼ぶ
- **THEN** Agent Service は domain handling 前にリクエストを拒否する
- **AND** Agent-owned 状態は変更されない

#### Scenario: retiring key は制限された token window 内だけ検証される (AGENT-SECURITY-S012)
...
#### Scenario: メソッド scope matrix が不足 scope を拒否する (AGENT-SECURITY-S013)
...
- **THEN** Agent Service は `permission_denied` として拒否する
- **AND** Agent-owned 状態は変更されない
...
- **AND** 拒否理由は Agent scope 不一致として安全に分類される

#### Scenario: 再利用された `jti` は状態変更前に拒否される (AGENT-SECURITY-S015)
