...
- [ ] 2.12 Cloudflare Workers Connect adapter、router registration、generated service handlers、`AgentHealthService.Check`、`agent_id` から AIAgent stub へ向かう Worker-internal DO routing を実装する。完了条件は [AGENT-SECURITY-S009] と [AGENT-HEALTH-S001] が通ること。
...
- [ ] 4.1 `AIAgent` を Cloudflare Agents SDK class として拡張し、typed env bindings、Agent-local Queue entrypoints、Agent Connect facade からのみ到達する Worker-internal Durable Object RPC methods を実装する。完了条件は [AGENT-SECURITY-S009] が public DO RPC route 不在を確認すること。
...
- [ ] 4.11 `AgentHealthService.Check` が使う AIAgent-side health check command/query を実装し、credential や domain snapshot を返さない safe status に制限する。完了条件は tests が [AGENT-HEALTH-S001] と [AGENT-HEALTH-S002] を含むこと。
- [ ] 4.12 `AgentThreadService.ListThreads`、`GetThread`、`ListSections` handlers を実装し、Agent scope、Thread/Section authorization、pagination cursor scope、Section ordinal/range ordering、not found/error mapping を確認する。完了条件は [AGENT-EVENTING-S009] の tests が通ること。
- [ ] 4.13 `AgentStateService.GetState` と `GetConfig` handlers を実装し、lifecycle/current Run/scheduler/storage threshold/capability summary、current config version、policy metadata を secret-free snapshot として返す。完了条件は [AGENT-LIFECYCLE-S007] の tests が通ること。
...
- [ ] 6.10 `AgentThreadService.GetLatestCompaction`、`GetThreadMemory`、`SearchThreadHistory` handlers を実装し、latest ready compaction、active Memory version、History search filters、R2 reference digest/ownership metadata、running/failed output exclusion を確認する。完了条件は [AGENT-MEMORY-S008] の tests が通ること。
...
- [ ] 10.3 `packages/client/src/server/db/managed-agents.ts` repository に create/update/list/delete、pin/sort、rename、last-opened updates を実装する。完了条件は [CLIENT-REGISTRY-S001] が通ること。
...
- [ ] 12.14 `[AGENT-EVENTING-S008] Large Event payload is offloaded with digest metadata` を title に含む automated tests を追加する。
...
- [ ] 12.23 `[AGENT-MEMORY-S001] Compaction freezes one Section and opens the next` を title に含む automated tests を追加する。
- [ ] 12.24 `[AGENT-MEMORY-S002] Event arriving during compaction enters the open Section` を title に含む automated tests を追加する。
- [ ] 12.25 `[AGENT-MEMORY-S003] Compaction creates Handoff History and MemoryDelta` を title に含む automated tests を追加する。
- [ ] 12.26 `[AGENT-MEMORY-S004] Context Builder resumes from latest ready compaction and raw Events` を title に含む automated tests を追加する。
- [ ] 12.27 `[AGENT-MEMORY-S005] Memory update preserves provenance and version` を title に含む automated tests を追加する。
- [ ] 12.28 `[AGENT-MEMORY-S006] Large History body is stored in R2 with index metadata` を title に含む automated tests を追加する。
- [ ] 12.29 `[AGENT-MEMORY-S007] Memory rebase refreshes long-term Memory without losing lineage` を title に含む automated tests を追加する。
- [ ] 12.30 `[AGENT-SCHEDULE-S001] CreateSchedule requires a Thread context` を title に含む automated tests を追加する。
...
- [ ] 12.58 `[CLIENT-REGISTRY-S001] Managed Agent registry persists display and ordering metadata` を title に含む automated tests を追加する。
- [ ] 12.59 `[CLIENT-REGISTRY-S002] Credential reference stores no plaintext secret` を title に含む automated tests を追加する。
- [ ] 12.60 `[CLIENT-REGISTRY-S003] Server Action calls Agent RPC with generated Connect client` を title に含む automated tests を追加する。
- [ ] 12.61 `[CLIENT-REGISTRY-S004] Client reads Agent domain details from Agent RPC instead of D1 snapshots` を title に含む automated tests を追加する。
- [ ] 12.62 `[CLIENT-REGISTRY-S005] Client has no public Agent proxy route` を title に含む automated tests を追加する。
- [ ] 12.63 `[CLIENT-MANAGEMENT-S001] Agent list supports registry display and ordering` を title に含む automated tests を追加する。
- [ ] 12.64 `[CLIENT-MANAGEMENT-S002] Add Agent form validates connection metadata accessibly` を title に含む automated tests を追加する。
- [ ] 12.65 `[CLIENT-MANAGEMENT-S003] Agent overview renders server-side profile and config` を title に含む automated tests を追加する。
- [ ] 12.66 `[CLIENT-MANAGEMENT-S004] Settings screen updates config and rotates credential through Agent RPC` を title に含む automated tests を追加する。
...
- [ ] 12.77 `[AGENT-MEMORY-S008] Thread memory and history queries return scoped references` を title に含む automated tests を追加する。
