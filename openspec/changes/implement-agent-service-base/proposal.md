...
- foundation-only の package restructure、demo removal、Protobuf/Connect guardrail scaffold、`agent-adapter.tsp` の service file 作成を重複実装しない。この change はそれらを前提に、Agent lifecycle、Thread/Event、Run、Compaction/Memory、Schedule、Tool、Integration、Client 管理 UI の Stage 1〜8 機能挙動を実装する。
...
- Schedule、ToolInvocation、Approval、Integration Installation、Adapter Connection、DeliveryContext/AdapterDelivery を AIAgent Durable Object が所有する。
- Provider-facing RPC contract として `IntegrationToolService.InvokeTool/GetOperation/CancelOperation` と `IntegrationDeliveryService.Deliver` を TypeSpec/Protobuf で定義し、Agent から Provider への Tool/Delivery 呼び出しも generated client 経由の signed Connect + binary Protobuf に統一する。
...
- Client 管理 UI は Agent 一覧、Agent profile、Thread/Event/Run/Compaction、Schedule、Tool approval、Integration install/uninstall、Agent config を server-side Agent RPC 経由で操作する。
...
- `agent-schedule`: New。Agent-owned Schedule、thread-scoped trigger Event、overlap policy、Integration uninstall 時の cancel を定義する。
...
- `agent-integration`: New。Integration manifest、Installation lifecycle、`AgentIntegrationService.CreateAdapterConnection/DeleteAdapterConnection/ListAdapterConnections` による Adapter Connection 管理、IntegrationIngressService、Delivery RPC、uninstall cleanup、Installation signature を定義する。
- `agent-security`: New。Client Service JWT、Integration detached signature、raw body digest、nonce/idempotency replay protection、final Agent-local authorization、Connect error mapping、observability を定義する。
...
- `client-management`: New。Next.js 管理 UI の Agent/Thread/Event/Run/Compaction/Schedule/Tool/Integration/Settings 画面、Browser credential 非公開、Agent proxy 非提供を定義する。
...
- `agent-schedule` は `AGENT-SCHEDULE`、`agent-tool` は `AGENT-TOOL`、`agent-integration` は `AGENT-INTEGRATION`、`agent-security` は `AGENT-SECURITY`、`agent-health` は `AGENT-HEALTH` を Scenario ID prefix とする。
